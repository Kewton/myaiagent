...
@tool
def google_search_tool(query: str) -> GoogleSearchResult:
    ...
    Returns:
        GoogleSearchResult: 検索結果を含むpydanticモデル。
              - result (list): Gemini APIから返されたテキストと参照されたURIから取得したHTMLをマークダウンファイル化したもの。
...
def googleSearchAgent(_input: str) -> GoogleSearchResult:
    """Google Searchを用いて情報を取得し、結果を返す。
