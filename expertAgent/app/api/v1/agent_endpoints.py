...
from aiagent.langgraph.jsonoutputagent.jsonOutputagent import jsonOutputagent
...
@router.post("/aiagent/jsonoutput",
             summary="LangGraphのAIエージェントを実行します",
             description="LangGraphのAIエージェントを実行します")
async def aiagent_jsonOutput(request: ExpertAiAgentRequest):
    try:
        _input = f"""
        # メタ情報:
        - 現在の時刻は「{datetime.now()}」です。
